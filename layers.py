...
class FullyConnected(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        Z = X @ self.parameters["W"]
        B = np.zeros(Z.shape)
        for i in range(B.shape[0]):
            B[i, :] =  self.parameters["b"]
        Z += B
...
class Elman(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        self.parameters = OrderedDict({"W": W, "U": U, "b": b})
        self.gradients = OrderedDict({"W": None, "U": None, "b": None})
    ...
    def forward_step(self, X: np.ndarray) -> np.ndarray:
        ...
        Z = X @ self.parameters["W"] + self.cache["s"][-1] @ self.parameters["U"]
        B = np.zeros(Z.shape)
        for i in range(B.shape[0]):
            B[i, :] =  self.parameters["b"]
        Z += B
    ...
    def backward(self, dLdY: np.ndarray) -> List[np.ndarray]:
        ...
        for i in reversed(range(len(X))):
            dLdZ = self.activation.backward(Z[i], dLds)
            dLdW += X[i].T @ dLdZ
            dLdU += s[i].T @ dLdZ
            dLdb += np.ones(dLdZ.shape[0]) @ dLdZ
            dLdX.append(dLdZ @ self.parameters["W"].T)
            dLds = dLdZ @ self.parameters["U"].T
...
class Conv2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        Z = np.zeros((n_examples, in_rows, in_cols, out_channels))
                    ...
                    for n in range(Z.shape[3]):
                        for i in range(kernel_height):
                            for j in range(kernel_width):
                                for c in range(in_channels):x
                                    Z[batch, d1, d2, n] += W[i, j, c, n] * X[batch, d1 + i, d2 + j, c] + b[0, n]
        out = self.activation.forward(Z)
